(function () {
  ...
  function setLoggedInHeaderProfileButton(button, profile) {
    ...
    button.onclick = null;
    button.setAttribute("aria-label", `Meu perfil (${profile.nome || "usuário"})`);
  }
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        return {
          ...
          cidade: dados.cidade || "",
          fotoUrl: dados.fotoPerfil || dados.fotoUrl || user.photoURL || ""
        };
      }
    } catch (error) {
  ...
  function setLoggedInButton(button, profile) {
    ...
    // O botão do menu deve levar diretamente para a página completa do usuário.
    // O card rápido continua disponível para compatibilidade, mas não intercepta o clique.
    button.onclick = null;
  }
  ...
  function setLoggedOutHeroAvatarButton(button) {
    ...
    icon.innerHTML = '<circle cx="12" cy="8" r="3.5"></circle><path d="M4.5 20a7.5 7.5 0 0 1 15 0"></path>';
  ...
  function setLoggedInHeroAvatarButton(button, profile) {
    ...
    if (photoUrl) {
      ...
      const initials = document.createElement("span");
      initials.className = "profile-avatar";
      initials.style.cssText = "width:100%;height:100%;border:none;";
      initials.textContent = getInitials(profile.nome);
    ...
    // Avatar do cabeçalho/hero também abre a página completa do perfil.
    button.onclick = null;
  }

  function toggleHeroNavMode(loggedIn) {
    const guestMenu = document.getElementById("heroGuestMenu");
    const iconNavbar = document.getElementById("heroIconNavbar");
    if (guestMenu) guestMenu.style.display = loggedIn ? "none" : "";
    if (iconNavbar) iconNavbar.style.display = loggedIn ? "flex" : "none";
  }
  ...
  function setupAuthProfileButton() {
    ...
    if (avatarButton) setLoggedOutHeroAvatarButton(avatarButton);
    if (headerButton) setLoggedOutHeaderProfileButton(headerButton);
    ...
    import("https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js")
      .then(({ onAuthStateChanged }) => {
        onAuthStateChanged(window.firebaseAuth, async (user) => {
          ...
          if (avatarButton) setLoggedInHeroAvatarButton(avatarButton, profile);
          if (headerButton) setLoggedInHeaderProfileButton(headerButton, profile);
        });
      })
      .catch((error) => {
